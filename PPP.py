...
def train_GBM_model(train_data, test_data):
    # Prepare training and test data
    X_train = train_data.drop(columns=['PPP_VALUE', 'geo', 'STRUCTURE', 'STRUCTURE_ID', 'na_item', 'ppp_cat', 'OBS_FLAG', 'country_name'])
    y_train = train_data['PPP_VALUE']
    ...
    # Calculate effectiveness in % using R²
    ...
    print(f"Root Mean Squared Error (RMSE) on validation data: {rmse}")
    print(f"R² (R-squared) on validation data: {r2}")
    print(f"R² as percentage: {r2_percentage}%")
    print(f"Mean Absolute Percentage Error (MAPE) on validation data: {mape}%")
    ...
    return gbm, mse, mae, rmse, r2, r2_percentage, mape
...
if train_data is None or test_data is None:
    ...
    best_model, mse, mae, rmse, r2, cv_scores = train_GBM_model(train_data, test_data)
